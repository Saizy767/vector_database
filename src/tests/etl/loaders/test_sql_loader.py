...
def test_bulk_insert(setup_db: SQLConnector):
    loader = SQLLoader(connector = setup_db,
                       ...
                       orm_class=Book)
    data = [
        {"book_id": 1, "title": "Book A", "author": "Author 1"},
        {"book_id": 2, "title": "Book B", "author": "Author 2"}
    ]
    loader.load(data)

    with setup_db.connect() as session:
        result = session.execute(
            text(f"SELECT * FROM {Book.table_name} ORDER BY book_id")
            ).mappings().all()
        assert len(result) == 2
...
def test_upsert(setup_db):
    loader = SQLLoader(
        connector=setup_db,
        table_name=Book.table_name,
        conflict_update=['title', 'author'],
        conflict_target=['book_id']
    )

    # Вставляем существующую книгу с новым названием
    data = [{"book_id": 1, "title": "Updated Book A", "author": "Updated Author 1"}]
    loader.load(data)
    
    with setup_db.connect() as session:
        result = session.execute(text(f"SELECT * FROM {Book.table_name} WHERE book_id=1")).first()
        assert result.title == "Updated Book A"
        assert result.author == "Updated Author 1"


def test_empty_data(setup_db):
...
def test_batch_insert(setup_db):
    loader = SQLLoader(connector=setup_db, table_name=Book.table_name, orm_class=Book, batch_size=1)
    data = [
        {"book_id": 3, "title": "Book C", "author": "Author 3"},
        {"book_id": 4, "title": "Book D", "author": "Author 4"}
    ]
    loader.load(data)

    with setup_db.connect() as session:
        result = session.execute(
            text(f"SELECT * FROM {Book.table_name} WHERE book_id IN (3,4)")).all()
        assert len(result) == 2
